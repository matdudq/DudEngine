...
VkResult SwapChain::SubmitCommandBuffers(
  ...
  API_CALL_VALIDATE(vkQueueSubmit(device.GraphicsQueue(),
								  1,
								  &submitInfo,
								  inFlightFences[currentFrame]));
...
void SwapChain::CreateSwapChain() {
  ...
  API_CALL_VALIDATE(vkCreateSwapchainKHR(device.GetDevice(),
										 &createInfo,
										 nullptr,
										 &swapChain));
  ...
  vkGetSwapchainImagesKHR(device.GetDevice(),
						  swapChain,
						  &imageCount,
...
void SwapChain::CreateImageViews() {
  ...
  for (size_t i = 0; i < swapChainImages.size(); i++) {
	...
	API_CALL_VALIDATE(vkCreateImageView(device.GetDevice(),
										&viewInfo,
										nullptr,
...
void SwapChain::CreateRenderPass() {
  ...
  API_CALL_VALIDATE(vkCreateRenderPass(device.GetDevice(),
									   &renderPassInfo,
									   nullptr,
									   &renderPass));
}
...
void SwapChain::CreateSyncObjects() {
  ...
  for (size_t i = 0; i < maxFramesInFlight; i++) {
	API_CALL_VALIDATE(vkCreateSemaphore(device.GetDevice(),
										&semaphoreInfo,
										nullptr,
										&imageAvailableSemaphores[i]));
	API_CALL_VALIDATE(vkCreateSemaphore(device.GetDevice(),
										...
										nullptr,
										&renderFinishedSemaphores[i]));
	API_CALL_VALIDATE(vkCreateFence(device.GetDevice(),
									&fenceInfo,
									nullptr,
									&inFlightFences[i]));
  }
...
VkSurfaceFormatKHR SwapChain::ChooseSwapSurfaceFormat(const std::vector<
